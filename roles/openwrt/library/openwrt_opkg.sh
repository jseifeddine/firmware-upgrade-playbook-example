...
query_package() {
    [ -n "$(opkg-solo status "$1")" ]
}

install_packages() {
    local _IFS pkg pkg_name
    ...
    for pkg; do
        ...
        [ -n "$_ansible_check_mode" ] || {
            try opkg-solo install$force $nodeps "$pkg"
...
remove_packages() {
    ...
    for pkg; do
        ...
        [ -n "$_ansible_check_mode" ] || {
            try opkg-solo remove$force $autoremove $nodeps "$pkg"
...
main() {
    ...
    [ -z "$update_cache" -o -n "$_ansible_check_mode" ] || try opkg-solo update
    case "$state" in
